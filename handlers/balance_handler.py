...
from database import (get_user_balance, get_exchange_rate, get_user_language, update_balance, get_admins,
                      get_user_balance_perevod, get_user_for_perevod, update_user_balance_perevod,load_texts,get_user_balance_perevod_by_id,get_username_by_id,
                      get_ved_exchange_rate)# Ваши функции работы с БД
...
async def get_exchange_rates():
    """ Получает текущие курсы валют из Binance в пересчёте на USD """
    url = "https://api.binance.com/api/v3/ticker/price"
    symbols = ["BTCUSDT", "BTCRUB"]  # BTC к USDT и BTC к RUB
    rates = {}
        ...
        try:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    # BTC к USD (USDT)
                    rates["BTC/USD"] = prices.get("BTCUSDT", 0)

                    # USDT к USD
                    rates["USDT/USD"] = 1  # USDT = 1 USD

                    # RUB к USD через BTC
                    btc_usdt = prices.get("BTCUSDT", 1)
                    btc_rub = prices.get("BTCRUB", 0)
                    rates["RUB/USD"] = btc_rub / btc_usdt if btc_usdt else 0
                else:
                    logger.warning(f"Ошибка при получении курсов: {response.status}")
        except Exception as e:
            logger.error(f"Ошибка запроса к API Binance: {e}")
...
async def balance_button_handler(message: types.Message, state: FSMContext):
    ...
    # Получаем текущие курсы (например, из API или другой базы данных)
    exchange_rates = await get_exchange_rates()

    # Логируем полученные курсы
    btc_usd = exchange_rates.get("BTC/USD", "N/A")
    usdt_usd = exchange_rates.get("USDT/USD", "N/A")
    rub_usd = exchange_rates.get("RUB/USD", "N/A")
    logger.debug(f"Курсы валют: BTC/USD = {btc_usd}, USDT/USD = {usdt_usd}, RUB/USD = {rub_usd}")

    # Получаем курс VED из базы данных и приводим к числовому типу для корректного отображения
    ...
    exchange_message = texts['exchange_rates_message'][user_language].format(
        ved=ved_rate,  # Берем курс VED из БД
        btc=btc_usd,
        usdt=usdt_usd,
        rub=rub_usd
    )
    ...
    # Отправка сообщений с курсами и балансом
    await message.answer(exchange_message)
