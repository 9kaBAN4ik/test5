...
from database import (get_user_courses, get_course_progress, get_user_products,get_user_role,get_product_by_id,
                      ...
                      connect_db,update_user_language,load_texts,get_user_language,get_course_by_id,add_course,add_lesson,get_random_image,get_random_aphorism,has_educational_module)# Функции для работы с БД
import logging
import json
...
# Внесём изменения в состояния для администраторов
class LotteryStatesForFriend(StatesGroup):
    awaiting_user_info = State()  # Ожидание информации о пользователе (username или user_id)
    awaiting_ticket_number = State()  # Ожидание номера билета
...
async def personal_info_menu(message: types.Message, state: FSMContext):
    ...
    buttons = [
        ...
        [KeyboardButton(text=texts['lottery_menu_button'][user_language])],  # Кнопка для входа в подменю лотереи
        [KeyboardButton(text=texts['language_button'][user_language])],
    ]

    if user_role in ['partner', 'admin']:
        has_course = await has_educational_module(user_id)  # Проверяем наличие курса
    ...
    await message.answer(
        texts['select_section'][user_language],  # Текст в зависимости от языка
        reply_markup=keyboard
    )


async def users_lottery_menu(message: types.Message, state: FSMContext):
    user_language = await get_user_language(message.from_user.id)
    ...
    buttons = [
        ...
        [KeyboardButton(text=texts['participate_in_the_lottery_button'][user_language]),
         ...
         KeyboardButton(text=texts['buy_ticket_for_another_user_button'][user_language])],
        [KeyboardButton(text=texts['back_to_personal_menu_button'][user_language])]
    ]

    keyboard = ReplyKeyboardMarkup(keyboard=buttons, resize_keyboard=True)

    await message.answer(
        texts['lottery_section'][user_language],  # Текст для подменю лотереи
        reply_markup=keyboard
    )
...
@router.message(lambda message: message.text and message.text.strip() in {str(texts['lottery_menu_button'].get(lang, '') or '').strip() for lang in texts['lottery_menu_button']})
async def open_lottery_menu(message: types.Message, state: FSMContext):
    await users_lottery_menu(message, state)

@router.message(lambda message: message.text and message.text.strip() in {str(texts['back_to_personal_menu_button'].get(lang, '') or '').strip() for lang in texts['back_to_personal_menu_button']})
async def back_to_personal_menu(message: types.Message, state: FSMContext):
    await personal_info_menu(message, state)
...
@router.message(lambda message: message.text and message.text.strip() in {
    str(texts['participate_in_the_lottery_button'].get(lang, '') or '').strip() for lang in
    texts['participate_in_the_lottery_button']})
async def participate_lottery(message: types.Message):
    user_id = message.from_user.id
    ...
    with connect_db() as conn:
        ...
        cursor.execute("SELECT id, ticket_price, fund FROM lottery WHERE active = 1")
        ...
        lottery_id, ticket_price, fund = lottery
        ...
        # Обновляем баланс пользователя (- стоимость билета)
        cursor.execute("UPDATE users SET balance = balance - ? WHERE user_id = ?", (ticket_price, user_id))

        # Обновляем призовой фонд (+ стоимость билета)
        cursor.execute("UPDATE lottery SET fund = fund + ? WHERE id = ?", (ticket_price, lottery_id))

        # Получаем следующий номер билета
        cursor.execute("SELECT COUNT(*) FROM lottery_tickets WHERE lottery_id = ?", (lottery_id,))
        ticket_number = cursor.fetchone()[0] + 1  # Следующий номер билета

        # Добавляем новый билет
        cursor.execute(
            "INSERT INTO lottery_tickets (lottery_id, user_id, username, ticket_number) VALUES (?, ?, ?, ?)",
            (lottery_id, user_id, username, ticket_number)
        )
        conn.commit()
...
async def view_my_tickets(message: types.Message):
    ...
    if tickets:
        response = "\n".join([f"Билет {ticket[0]}" for ticket in tickets])
    else:
...
@router.message(lambda message: message.text and message.text.strip() in {str(texts['buy_ticket_for_another_user_button'].get(lang, '') or '').strip() for lang in texts['buy_ticket_for_another_user_button']})
async def buy_ticket_for_another_user(message: types.Message, state: FSMContext):
    user_id = message.from_user.id

    # Запрашиваем у администратора, для кого он хочет купить билет
    await message.reply("Введите username или user_id пользователя, для которого вы хотите купить билет:")
    await state.set_state(LotteryStatesForFriend.awaiting_user_info)

@router.message(LotteryStatesForFriend.awaiting_user_info)
async def process_user_info(message: types.Message, state: FSMContext):
    ...
    with connect_db() as conn:
        ...
        # Проверяем, введён ли user_id или username
        if user_info.isdigit():
            target_user_id = int(user_info)
            target_username = None
        else:
            cursor.execute("SELECT user_id, username FROM users WHERE username = ?", (user_info,))
        ...
        if not admin or admin[0] < ticket_price:
            await message.reply("Недостаточно средств на балансе для покупки билета.")
            return

        # Списание средств с баланса администратора
        cursor.execute("UPDATE users SET balance = balance - ? WHERE user_id = ?", (ticket_price, admin_id))

        # Генерируем новый номер билета
        cursor.execute("SELECT COUNT(*) FROM lottery_tickets WHERE lottery_id = ?", (lottery_id,))
        ticket_number = cursor.fetchone()[0] + 1  # Новый билет получает следующий номер

        # Добавляем новый билет в базу
        cursor.execute(
            "INSERT INTO lottery_tickets (lottery_id, user_id, username, ticket_number) VALUES (?, ?, ?, ?)",
            (lottery_id, target_user_id, target_username, ticket_number)
        )
    ...
    await state.clear()
    await message.reply(f"Вы успешно приобрели билет #{ticket_number} для пользователя с ID {target_user_id}.")
