...
@router.message(lambda message: message.text and message.text.strip().lower() == "отмена")
...
async def add_after_purchase_info(message: Message, state: FSMContext):
    ...
    await message.answer(texts['add_product_tags'].get(user_language, texts['add_product_tags']['en']), reply_markup=cancel_keyboard)
...
async def set_product_visibility(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Это образовательный модуль?", reply_markup=keyboard)
...
async def buy_product_handler(callback: CallbackQuery):
    await callback.answer()  # Подтверждаем запрос пользователя
    product_id = int(callback.data.split("_")[2])  # Извлекаем ID продукта
    user_language = str(callback.data.split("_")[3])
    user_id = callback.from_user.id
    ...
    if product:
        if user_balance >= product["price"]:
            ...
            if referral_system_id:
                ...
                # Получаем уровень текущего пользователя
                cursor.execute("SELECT level FROM referrals WHERE referred_id = ?", (user_id,))
                user_referral_data = cursor.fetchone()

                if user_referral_data:
                    user_level = user_referral_data[0]
                    current_id = user_id
                    current_level = user_level

                    # Проходим по всей цепочке рефералов
                    while current_level > 0:
                        cursor.execute("SELECT referrer_id, level FROM referrals WHERE referred_id = ?", (current_id,))
                        referrer_data = cursor.fetchone()

                        if not referrer_data:
                            break  # Нет реферера — выходим из цикла

                        referrer_id, referrer_level = referrer_data
                        reward_percentage = get_referral_reward(referral_system_id, referrer_level)
                        reward_amount = (product["price"] * reward_percentage) / 100

                        if reward_amount > 0:
                            # Обновляем баланс реферера
                            referrer_balance = await get_user_balance(referrer_id)  # Получаем текущий баланс реферера
                            new_referrer_balance = referrer_balance + reward_amount  # Прибавляем награду
                            update_user_balance(referrer_id, new_referrer_balance)  # Обновляем баланс в БД

                            # Добавляем запись о реферальной награде
                            add_referral_reward(referrer_id, reward_amount)

                            # Отправляем уведомление рефереру
                            try:
                                message_text = (
                                    f"🎉 Вы получили {reward_amount} VED за покупку, совершённую вашим рефералом!\n"
                                    f"💰 Ваш новый баланс: {new_referrer_balance} VED"
                                    if user_language == "ru"
                                    else f"🎉 You received {reward_amount} VED for a purchase made by your referral!\n"
                                         f"💰 Your new balance: {new_referrer_balance} VED"
                                )
                                await callback.bot.send_message(chat_id=referrer_id, text=message_text)
                            except Exception as e:
                                print(f"Не удалось отправить сообщение рефереру {referrer_id}: {e}")

                        # Переход к следующему уровню реферала
            ...
            if partner_id:
                try:
                    # Получаем текущий баланс партнёра
                    partner_balance = await get_user_balance(partner_id)
                    new_partner_balance = partner_balance + product["price"]  # Добавляем сумму покупки

                    # Обновляем баланс партнёра в БД
                    update_user_balance(partner_id, new_partner_balance)

                    # Уведомляем партнёра о продаже
                    author_message = (
                        f"🔔 Ваш продукт был куплен!\n"
                        f"📦 Продукт: {product['name']}\n"
                        f"👤 Покупатель: {callback.from_user.full_name} (ID: {user_id})\n"
                        f"💰 Сумма: {product['price']} VED\n"
                        f"💰 Ваш новый баланс: {new_partner_balance} VED"
                    )
