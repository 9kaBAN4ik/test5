...
def get_username_by_user_id_for_admin(user_id):
    # Получаем имя пользователя по user_id, можно запросить из базы данных
    with connect_db() as conn:
        cursor = conn.cursor()
...
def get_users_with_balance():
    with connect_db() as conn:
        ...
        return [dict(zip(["user_id", "username", "balance"], row)) for row in cursor.fetchall()]


def get_user_id_for_ticket(ticket_number):
    with connect_db() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT user_id FROM lottery_tickets WHERE ticket_number = ?", (ticket_number,))
        result = cursor.fetchone()

        # Если билет найден, возвращаем user_id
        if result:
            return result[0]
        else:
            # Если билет не найден, возвращаем None или выбрасываем ошибку
            return None

async def has_educational_module(partner_id: int) -> bool:
    query = "SELECT EXISTS(SELECT 1 FROM courses WHERE partner_id = ?)"
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(query, (partner_id,)) as cursor:
            result = await cursor.fetchone()
            return result[0] == 1  # Вернёт True, если есть курс

async def get_ved_exchange_rate():
    """Получает курс VED из базы данных"""
    query = "SELECT rate FROM exchange_rates WHERE currency = 'VED'"
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(query) as cursor:
            row = await cursor.fetchone()
            print(f"Запрос к БД вернул: {row}")  # Логируем результат
