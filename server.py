...
async def end_lottery(message: types.Message):
    ...
    with connect_db() as conn:
        ...
        referral_fund = (total_fund * referral_percentage) / 100  # Фонд для рефералов
    ...
    for ticket in tickets:
        ...
        for level in range(1, len(reward_percentages) + 1):
            with connect_db() as conn:
                cursor = conn.cursor()
                cursor.execute(""" 
            ...
            if not referrer or not referrer[0]:  # Если реферера нет, прекращаем цепочку
                break
